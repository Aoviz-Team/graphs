...
export function generateText(value: string, style: Partial<ICanvasFontStyle> = {}) {
  ...
  const width = Math.abs(Math.cos(Math.abs(radian)) * textMeasured.width + fontStyle.size * Math.sin(Math.abs(radian)));
  const height = Math.abs(Math.sin(Math.abs(radian)) * textMeasured.width + fontStyle.size * Math.cos(Math.abs(radian)));
